...
def parallelize_transformer(pipe: DiffusionPipeline):
    ...
    def new_forward(
        ...
        post_patch_num_frames = num_frames // p_t  #5   frame 高 宽 分块数
        post_patch_height = height // p_h          #45
        post_patch_width = width // p_w            #80

        rotary_emb = self.rope(hidden_states)
        #[2, 16, 5, 90, 160] -> [2, 18000, 5120]
        ...
        #encoder_hidden_states [2, 512, 5120]
        ...
        #rope操作 hidden_states  [1, 4500, 5120] -> q矩阵 [2, 40, 18000, 64, 2] 如果ulysses=4 18000->4500 [batch, 多头头数, 序列长度, 每个 头的特征维度, 2] 2表示实部虚部
        #rope操作 cos [1, 1, 18000, 64] 其中18000=ppf * pph * ppw  frame high width每个维度的分块数量相乘
        #usp把q矩阵 [2, 40, 18000, 64, 2]拆成四个[2, 40, 4500, 64, 2], rope freq的18000也被分成四份
    ...
    #把WanTransformerBlock里的self(attn1) attention和cross(attn2) attention的processor都替换
...
def main():
    ...
    if hasattr(torch.cuda.nccl, "set_log_level"):
        torch.cuda.nccl.set_log_level(2)  # 2 对应 NCCL_LOG_ERROR
    init_distributed_environment(
        rank=dist.get_rank(),
    ...
    #initialize_model_parallel(
    #    ring_degree=engine_config.parallel_config.sp_config.ring_degree,
    #    ulysses_degree=engine_config.parallel_config.sp_config.ulysses_degree,
    #    classifier_free_guidance_degree=2 if engine_config.parallel_config.dp_config.use_cfg_parallel == True else 1,
    #)
