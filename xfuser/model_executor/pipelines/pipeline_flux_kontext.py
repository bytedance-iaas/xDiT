...
class xFuserFluxKontextPipeline(xFuserPipelineBaseWrapper):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if engine_config.use_svdq:
            ...
            pipeline = FluxKontextPipeline.from_pretrained(pretrained_model_name_or_path, transformer=transformer, **kwargs)
            ...
            pipeline = FluxKontextPipeline.from_pretrained(pretrained_model_name_or_path, **kwargs)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        target_length = (int(math.ceil(latent_ids.size(0) / get_sequence_parallel_world_size()))
                         * get_sequence_parallel_world_size())
        padding = target_length - latent_ids.size(0)

        latent_ids = torch.cat([
            latent_ids,
            torch.zeros(padding, latent_ids.size(1),
                        dtype=latent_ids.dtype,
                        device=latent_ids.device)
        ], dim=0)
        ...
        target_length = (int(math.ceil(latent_model_input.size(1) / get_sequence_parallel_world_size()))
                         * get_sequence_parallel_world_size())
        padding = target_length - latent_model_input.size(1)

        latent_model_input = torch.cat([
            latent_model_input,
            torch.zeros(latent_model_input.size(0), padding, latent_model_input.size(2),
...
def parallelize_transformer(pipe: DiffusionPipeline):
    ...
    def new_forward(
        ...
        if isinstance(timestep, torch.Tensor) and timestep.ndim != 0 and timestep.shape[0] == hidden_states.shape[
            0]:
        ...
        torch.chunk(encoder_hidden_states, get_classifier_free_guidance_world_size(), dim=0)[
            get_classifier_free_guidance_rank()]
