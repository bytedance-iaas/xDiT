...
class VideoGenerator:
    ...
    def initialize_model(self, xfuser_args: xFuserArgs):
        ...
        pipeline_map = {
            "Wan2.1-I2V-14B-720P-Diffusers": xFuserWanImageToVideoPipeline,
            "Wan2.2-I2V-A14B-Diffusers": xFuserWanImageToVideoPipeline,
        }
        ...
        text_encoder = UMT5EncoderModel.from_pretrained(self.engine_config.model_config.model,
        ...
        if "Wan2.1" in model_name:
            from transformers import CLIPVisionModel
            image_encoder = CLIPVisionModel.from_pretrained(xfuser_args.model, subfolder="image_encoder",
                                                        torch_dtype=torch.float32)
        ...
        if xfuser_args.enable_quantize:
            self.logger.info(f"doing quantize model")
        ...
        if xfuser_args.enable_model_cpu_offload:
            ...
            if not xfuser_args.enable_quantize:
                apply_group_offloading(text_encoder, onload_device=onload_device, offload_device=offload_device,
                                       offload_type="leaf_level")
            if "Wan2.1" in model_name:
                apply_group_offloading(image_encoder, onload_device=onload_device, offload_device=offload_device,
                                       offload_type="leaf_level")
        ...
        image = load_image("./entrypoints/image_examples/i2v_input.JPG")
        max_area = 720 * 1280
    ...
    def generate(self, request: GenerateRequest):
        def _process_image(self, base64_str: str):
            self.logger.info("WanI2V processing input image")
            from PIL import Image
            from io import BytesIO
            if not isinstance(base64_str, str):
                self.logger.error("base64_str is not str")
                print("base64_str is not str")
            try:
            ...
            try:
                with Image.open(buffer) as img:
                    ...
                    self.logger.info(f"Image size: {rgb_img.size}")
            except Exception as e:
        ...
        try:
            ...
            image = _process_image(self, request.image)
            image_width, image_height = image.size
            max_area = request.height * request.width
            aspect_ratio = image_height / image_width
            mod_value = self.pipe.vae_scale_factor_spatial * self.pipe.transformer.config.patch_size[1]
            height = round(np.sqrt(max_area * aspect_ratio)) // mod_value * mod_value
            width = round(np.sqrt(max_area / aspect_ratio)) // mod_value * mod_value
            self.logger.info(f"resize image to {width} x {height}")
            image = image.resize((width, height))
            ...
            if self.pipe.is_dp_last_group():
                ...
                self.logger.info(f"Writing frames to bytes buffer elapsed {write_elapsed:.2f} s")
